...
const SKIP_DIRS: &[&str] = &[
    ...
    "vendor", ".next", "out", "coverage",
];

fn is_code_file(name: &str) -> bool {
    matches!(
        name.rsplit('.').next(),
        Some("rs" | "py" | "ts" | "tsx" | "js" | "jsx" | "go" | "rb" | "java" | "c" | "h" | "cpp" | "toml" | "yaml" | "yml" | "md")
    )
}
...
fn extract_rust_symbol(line: &str) -> Option<String> {
    for prefix in ["pub fn ", "pub async fn ", "fn ", "pub struct ", "pub enum ", "pub trait ", "impl "] {
        if let Some(rest) = line.strip_prefix(prefix) {
            return rest.split(['(', '<', ' ', '{', ':']).next()
                .filter(|s| !s.is_empty())
                .map(|s| format!("{}{s}", if prefix.contains("struct") { "struct " } else if prefix.contains("enum") { "enum " } else if prefix.contains("trait") { "trait " } else if prefix.contains("impl") { "impl " } else { "fn " }));
        }
    }
    None
}
...
fn extract_ts_symbol(line: &str) -> Option<String> {
    for prefix in ["export function ", "export async function ", "export class ", "export interface ", "function "] {
        if let Some(rest) = line.strip_prefix(prefix) {
            return rest.split(['(', '<', ' ', '{', ':']).next()
                .filter(|s| !s.is_empty())
                .map(String::from);
...
mod tests {
    ...
    fn extract_rust_symbols() {
        assert_eq!(extract_rust_symbol("pub fn hello(x: i32)"), Some("fn hello".into()));
        assert_eq!(extract_rust_symbol("pub struct Foo {"), Some("struct Foo".into()));
    ...
    fn extract_python_symbols() {
        assert_eq!(extract_python_symbol("def main():"), Some("def main".into()));
        assert_eq!(extract_python_symbol("class MyApp(Base):"), Some("class MyApp".into()));
